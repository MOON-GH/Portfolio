		...
		$('#btnUpdate').on('click', function(){
			if("${sessionScope.memberId}" != null) {	// 로그인 유무 체크
				if(("${sessionScope.memberId}" == "${getBoard.memberId}") || "${sessionScope.typeSeq}" == "9"){		// 작성자확인 및 관리자 체크
					...
					if(confirm("수정하시겠습니까?")) movePage(this, "/board/goUpdate.do", {boardSeq:'${getBoard.boardSeq}', pageName:'boardUpdate'});
				} else {
		...
		$('#btnDelete').on('click', function(){
			...
			if("${sessionScope.memberId}" != null) {	// 로그인 유무 체크
				if(("${sessionScope.memberId}" == "${getBoard.memberId}") || "${sessionScope.typeSeq}" == "9"){		// 작성자확인 및 관리자 체크
			   	...
			   	if(confirm("게시글을 삭제하시겠습니까?"))	customAjax("<c:url value='/board/delete.do' />", "/board/list.do?page=${page}");
				} else {
		...
		$.ajax({
			...
			success : function (result, textStatus, XMLHttpRequest) {
		        ...
		        alert(data.msg);
				    ...
				    if(data.result == 1){
			        movePage(this, movePageUrl, {pageName:'board'});
				    }
		                    ...
		                    <div class="col-sm-12" style="margin-top: 10px;">
		                    	<i class="glyphicon glyphicon-link"></i>&nbsp;<label for="exampleInputName2">첨부파일</label><br/>
								    ...
								    <div class="col-md-12 text-right">
											<c:if test="${(sessionScope.memberId != null && sessionScope.memberId == getBoard.memberId) || (sessionScope.typeSeq == '9')}">
							        ...
							        <a href="javascript:movePage(null, '/board/list.do', {page:'${page}', pageName:'board'})">
