		...
		$('#btnUpdate').on('click', function(){
			if("${sessionScope.memberId}" != null) {	// 로그인 유무 체크
		      ...
		      var formData = new FormData(frm);
			        ...
			        if(data.result == 1){
				        movePage(this, "/notice/read.do", {boardSeq:'${getBoard.boardSeq}', update:'update', pageName:'noticeRead'});
					    } else {
	...
	function deleteAttach(fileIdx, boardSeq) {
		$.ajax({
			...
			success : function (data, textStatus, XMLHttpRequest) {
		        ...
		        if(data.result){
			        movePage(this, "/notice/goUpdate.do", {boardSeq:'${getBoard.boardSeq}', pageName:'noticeUpdate'});
				    } else {
		          window.location.href="<c:url value='/index.do'/>";
				    }
			},
	                    	...
	                    	<c:forEach items="${files}" var="file" varStatus="vs">
								...
								<c:if test='${sessionScope.memberId != null }'>
									<div class="row">
								        ...
								        <a href="javascript:movePage(this, '/notice/list.do', {pageName:'notice'})">
